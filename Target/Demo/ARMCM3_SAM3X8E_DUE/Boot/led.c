...
{
  /* DONE ##Boot Configure the GPIO pin that the LED is connected to, as a digital output
...
void LedBlinkTask(void)
{
  ...
  if (TimerGet() >= nextBlinkEvent)
  {
    ...
    {
      ledOn = BLT_TRUE;
      /* DONE ##Boot Turn the LED on. */
      PIO_Set(PIOC, PIO_PC3);
    }
    else
    {
      ledOn = BLT_FALSE;
      /* DONE ##Boot Turn the LED off. */
...
{
  /* DONE ##Boot Turn the LED off and reset the GPIO pin configuration that was
   * configured to drive the LED.
   */
  PIO_Clear(PIOC, PIO_PC3);
  PIO_Configure(PIOC, PIO_INPUT, PIO_PC3, PIO_DEFAULT);
